...
fn load_settings_from_db(conn: &Connection) -> AppSettings {
    ...
    // Migration: if tts_voices_per_language is empty but legacy voice exists,
    // populate the current target_language entry with the legacy value
    if settings.tts_voices_per_language.is_empty()
        && !settings.tts_voice_id.is_empty()
        && !settings.target_language.is_empty()
    {
