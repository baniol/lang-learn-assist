...
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct TranscriptionWithAudio {
    pub transcription: String,
    pub saved_audio_path: String,
}

/// Transcribe audio and preserve the audio file for later playback
/// Unlike transcribe_audio, this command:
/// 1. Copies the audio to a permanent location (app_data/audio_segments/)
/// 2. Returns both the transcription and the saved audio path
#[tauri::command]
#[allow(non_snake_case)]
pub fn transcribe_and_preserve_audio(
    app: tauri::AppHandle,
    audioPath: String,
    language: Option<String>,
    state: State<'_, AppState>,
) -> Result<TranscriptionWithAudio, String> {
    let source_path = PathBuf::from(&audioPath);
    if !source_path.exists() {
        return Err(format!("Audio file not found: {audioPath}"));
    }
    ...
    let app_data_dir = app.path().app_data_dir().map_err(|e| e.to_string())?;
    let audio_dir = app_data_dir.join("audio_segments");
    std::fs::create_dir_all(&audio_dir).map_err(|e| format!("Failed to create audio dir: {e}"))?;

    // Generate unique filename and copy audio to permanent location
    let audio_filename = format!("{}.wav", Uuid::new_v4());
    let saved_path = audio_dir.join(&audio_filename);
    std::fs::copy(&source_path, &saved_path)
    ...
    // Clean up source audio file (the copy is preserved)
    let _ = std::fs::remove_file(&source_path);

    Ok(TranscriptionWithAudio {
        transcription: text.trim().to_string(),
        saved_audio_path: saved_path.to_string_lossy().to_string(),
    })
}
