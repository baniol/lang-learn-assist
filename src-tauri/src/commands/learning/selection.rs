...
///
/// Prioritizes:
/// 1. Overdue phrases (most overdue first)
/// 2. New phrases (up to new_phrase_limit)
/// 3. Skips phrases not yet due for review
#[tauri::command]
pub fn get_next_phrase(
    ...
    let mut phrases_with_priority: Vec<(PhraseWithProgress, f64)> = phrases
        ...
        .map(|p| {
            ...
            // New phrases have priority NEW_PHRASE (no progress or total_attempts == 0)
            if limit_new_phrases && (priority - NEW_PHRASE).abs() < 0.001 {
                priority = 0.0;
            }
