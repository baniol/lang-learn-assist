  ...
  if (phase === "exercise" && currentPhrase) {
    return (
      <ExerciseActivePhase
        ...
        onShowSentenceChange={(v) => {
          if (v === false) setAutoPlayAudio(true);
          setShowSentence(v);
        }}
        ...
        onPeekAnswer={handlePeekAnswer}
        onToggleStar={handleToggleStar}
        showEndConfirm={showEndConfirm}
        onEndConfirmOpen={() => setShowEndConfirm(true)}
        onEndConfirmClose={() => setShowEndConfirm(false)}
        onEndSession={() => {
...
// ============================================================================
// SUB-COMPONENTS (phase-specific, private to this module)
// ============================================================================

interface ExerciseSetupPhaseProps {
  tags: Tag[];
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="flex items-center bg-slate-100 dark:bg-slate-800 rounded-lg p-1">
            ...
            <button
              onClick={() => onInputModeChange("repeat")}
              className={cn(
                "flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-md text-sm font-medium transition-colors",
                inputMode === "repeat"
                  ? "bg-white dark:bg-slate-700 text-slate-800 dark:text-white shadow-sm"
...
interface ExerciseActivePhaseProps {
  currentPhrase: import("../types").ExerciseSessionPhrase;
  ...
  onShowSentenceChange: (v: boolean) => void;
  showingAnswer: boolean;
  voiceStatus: import("../hooks/useVoiceRecording").RecordingStatus;
  ...
  onEndConfirmClose: () => void;
  onEndSession: () => void;
  inputRef: React.RefObject<HTMLInputElement | null>;
}

function ExerciseActivePhase({
  currentPhrase,
  completedCount,
  totalCount,
  inputMode,
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-3">
              ...
              {inputMode === "repeat" && (
                <>
                  <button
                    onClick={() => onShowSentenceChange(!showSentence)}
                    className={cn(
              ...
              <Button variant="ghost" size="sm" onClick={onEndConfirmOpen}>
                End
              </Button>
            </div>
          </div>
...
interface ExerciseResultsPhaseProps {
  sessionPhrases: import("../types").ExerciseSessionPhrase[];
  completedCount: number;
  totalCount: number;
...
function ExerciseResultsPhase({
  ...
  return (
    <div className="p-6 max-w-xl mx-auto">
      <h1 className="text-2xl font-bold text-slate-800 dark:text-white mb-6">Exercise Complete</h1>
      ...
      <div className="flex gap-3">
        <Button onClick={onBackToSetup} variant="secondary" className="flex-1">
          Back to Setup
        </Button>
